			...
			<div v-if="baskets.length>0">
				<table class="table">
					...
					<tbody v-for="item in baskets" :key="item.name">
						<tr>
							<td>
								<button style="background: none;" class="btn btn-sm" @click="decreaseQuantity(item)">-</button>
								<span>{{item.quantity}}</span>
								<button style="background: none;" class="btn btn-sm" @click="increaseQuantity(item)">+</button>
							</td>
